...
from business_logic import get_eligables, get_eligable_info, eligable_list_to_dict, date_handler, yearify
...
@login_required
def index(request):
    # Get list of years relative to current year and list of departments
    now = datetime.now()
...
def add_schedule(request):
    ...
    try:
        ...
        schedule = Schedule(start_datetime=start, end_datetime=end,
        ...
        #return HttpResponseRedirect(reverse('schedulingcalendar:index'))
...
def get_schedule_info(request):
    ...
    pk = request.GET['pk']
    eligable_list = get_eligables(pk)
